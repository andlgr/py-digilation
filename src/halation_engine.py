...
class HalationEngine():
    ...
    def apply_advanced_halation(
            self,
            image,
            highlight_threshold=0.97,  # Still useful for optional flexibility
            red_boost=8,
            green_boost=1.5,
            blur_passes=[35, 55, 95],
            blend_strength=0.9,
            falloff_power=2.0,
            gamma=2.2
    ):
        ...
        # Create a mask where all channels are at full intensity (pure white)
        white_mask = np.all(img_lin >= 1.0, axis=2).astype(np.float32)

        # Optional: if you want to allow a tiny tolerance near 1.0
        tolerance = 5e-2
        white_mask = np.all(img_lin >= (1.0 - tolerance), axis=2).astype(np.float32)

        # Apply multi-scale Gaussian blur for glow effect.
        glow = np.zeros_like(white_mask)
        for size in blur_passes:
            blurred = cv2.GaussianBlur(white_mask, (size, size), 0)
            falloff = np.power(1.0 - blurred, falloff_power)
            glow += blurred * falloff
        glow /= len(blur_passes)

        # Build the red-channel halo (halation effect)
        halo = np.zeros_like(img_lin)
        halo[:, :, 2] = glow * red_boost  # Red
        halo[:, :, 1] = glow * green_boost  # Green
        ...
        return (final_srgb * 65535).astype(np.uint16), halo, white_mask, glow
